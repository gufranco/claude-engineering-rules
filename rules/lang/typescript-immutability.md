...
The hook runs as a static analyzer outside Claude Code when given `MUTATION_METHOD_BATCH_MODE=1` and `MUTATION_METHOD_OUTPUT=sarif`. Three reference snippets are below; pick the one that matches your CI provider.

**GitHub Actions.** Uploads SARIF to GitHub code scanning so findings appear inline on the PR.

```yaml
- name: Mutation method scan
  env:
    MUTATION_METHOD_BATCH_MODE: "1"
    MUTATION_METHOD_OUTPUT: "sarif"
    MUTATION_METHOD_FAIL_THRESHOLD: "warning"
  run: |
    git diff --name-only --diff-filter=AM "origin/${{ github.base_ref }}"...HEAD \
      ...
      > mutation-method-blocker.sarif
- uses: github/codeql-action/upload-sarif@v3
  with:
    sarif_file: mutation-method-blocker.sarif
    category: mutation-method-blocker
```

**GitLab CI.** Emits a SAST report under `reports.sast` so findings show up on the merge request widget.

```yaml
mutation-method-scan:
  stage: test
  ...
  variables:
    ...
    MUTATION_METHOD_FAIL_THRESHOLD: "warning"
  script:
    - git fetch --no-tags --depth=50 origin "$CI_MERGE_REQUEST_TARGET_BRANCH_NAME"
  ...
  artifacts:
    when: always
    reports:
      sast: mutation-method-blocker.sarif
    paths:
      - mutation-method-blocker.sarif
```

**CircleCI.** Stores the SARIF document as a build artifact; pair with a downstream uploader if your org centralizes findings.
  ...
  mutation-method-scan:
    ...
    steps:
      ...
      - run:
          ...
          command: |
            git diff --name-only --diff-filter=AM \
              "origin/$CIRCLE_BASE_REVISION"...HEAD \
              | grep -E '\.(ts|tsx|js|jsx|mjs|cjs)$' \
              | python3 hooks/mutation-method-blocker.py \
              > mutation-method-blocker.sarif
      - store_artifacts:
          path: mutation-method-blocker.sarif
          destination: mutation-method-blocker.sarif
```

## Type-Aware Detection

When `MUTATION_METHOD_TS_PROJECT_SERVICE=1` and Node.js with the `typescript` package (>= 5.6) are available, the hook spawns a long-running Node helper that uses the TypeScript Project Service to answer narrow type questions about each mutation site. The integration uses the type information for three categories:

| Receiver type | Hook behavior |
|---------------|---------------|
| `readonly T[]` / `ReadonlyArray<T>` / `ReadonlyMap` / `ReadonlySet` | Skip the finding. The compiler already rejects mutation |
| `URLSearchParams`, `Headers`, `FormData` | Keep the finding even when the source looks ambiguous |
| Any of the 12 `TypedArray` subtypes | Apply hot-path semantics regardless of directory |

The integration is optional. When the env var is unset or Node and typescript are not on PATH, the hook falls back to regex-only behavior with no penalty. Latency budget is 200ms per query, 5000ms total per invocation; the helper falls back silently on timeout.

```bash
MUTATION_METHOD_TS_PROJECT_SERVICE=1 python3 hooks/mutation-method-blocker.py
```

When `MUTATION_METHOD_DEBUG=1` is also set, the helper writes shutdown reasons to stderr (`ts-project-service: disabled (<reason>)`).

## Suppression Budget

Every `@claude-allow-mutation` and `claude-allow-mutation` marker is a deliberate escape hatch. Without a cap, allowlists accumulate silently and the rule erodes. The per-project budget config caps total markers, per-detector markers, and optionally requires a justification trailer (`-- <reason>`).

Place the config at `<project-root>/.claude/mutation-budget.yml` (or `.yaml` / `.json`). Schema lives at `~/.claude/schemas/mutation-budget.schema.json` (Draft-07).
...
```bash
python3 scripts/mutation_budget_check.py --root . --format text
# exit 0: in budget, 1: over budget, 2: invalid config
```

When `require_justification: true`, every marker must end with `-- <reason>`. Markers without a reason fail the budget check even if the count is within the cap.

## Detector Tuning

Quarterly cadence: run `scripts/detector_tuning_report.py` on the first day of each quarter, archive the output under `docs/telemetry/<YYYY-Q#>.md`, and tune the detectors with the worst signals.

The report reads telemetry from three sources:

| Source | Format | Flag |
|--------|--------|------|
| Hook audit log | JSON Lines at `~/.claude/logs/hooks.log` | `--audit` (default) |
| OpenTelemetry span dump | JSON Lines, one span per line, `hook.*` attributes | `--otel` |
| SARIF documents | One document per file in a directory | `--sarif` |

Detectors are flagged when:

- `allow_ratio > 0.5`: over half of hits are waved through by allowlists, scopes, or markers. The pattern is too broad
- `latency_p95 > 50ms`: the regex or scope check is expensive. Profile and rewrite
- `avg_confidence < 3.0`: the detector frequently produces low-confidence findings. Tighten or remove

```bash
python3 scripts/detector_tuning_report.py \
  --audit ~/.claude/logs/hooks.log --format text
python3 scripts/detector_tuning_report.py \
  --otel telemetry/hooks-2026-q2.jsonl --format json
```

The flag threshold requires at least 20 samples. Detectors with fewer samples are ignored to avoid noise.
...
See `checklists/checklist.md` category 5 (Data Integrity) for the matching boundary checks: write idempotency keys, transaction scope, and constraint alignment between validators and the database.
