...
- Treating a check that failed to run as a check that was skipped. A classifier that exits non-zero, a gate whose output will not parse, a state file that is missing, and a tool that errored are all failed checks, never absent ones. The asymmetry decides it: failing closed costs a false stop, which is visible and cheap to clear, while failing open costs an unverified change that looks verified. Full rule, including the exit-code and stream mechanics that make fail-closed real rather than nominal, in [`agent-operating-limits.md`](agent-operating-limits.md)
...
- Publishing a specific that a subagent, a search snippet, or a summary reported, without opening the primary source at that coordinate. Relays garble the coordinate, not only the content, so the quoted text reads correctly while the line number, timestamp, or identifier points somewhere else. See [`relay-not-source.md`](relay-not-source.md)
