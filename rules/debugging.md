...
- Common root causes: State (shared mutable state, stale cache, missing initialization), Timing (race condition, missing await, event ordering), Data (null/undefined, wrong type, encoding, timezone, off-by-one), Environment (missing env var, wrong version, different OS behavior), Integration (API contract changed, schema mismatch, network timeout)
...
Track failed attempts in a structured table: Attempt | What was tried | Error | Next action.
...
- **Workarounds over fixes**: setting an env var to suppress a deprecation warning instead of upgrading the dependency. Adding a compatibility shim instead of migrating the caller. Every workaround is technical debt. If an upstream fix exists, apply it. Only accept a workaround when no permanent fix is available, and document why with a `TODO(debt):` comment.
- **Relaxing the checker**: when a linter or CI check fails, fix the code. Do not lower severity, disable rules, or weaken configuration. Only suppress a finding after confirming it is a false positive or a deliberate, documented pattern.
