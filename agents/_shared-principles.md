...
The [`agents/`](agents) directory contains both agent definitions and support files. Counts in documentation refer to **agents only**, excluding files prefixed with `_` (shared fragments) and [`TEMPLATE.md`](agents/TEMPLATE.md) (boilerplate). To count agents: `ls agents/*.md | grep -vE "^_|TEMPLATE"`.
