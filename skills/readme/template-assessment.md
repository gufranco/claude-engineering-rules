...
The technical-assessment variant of `/readme`. Use when the README needs to demonstrate engineering depth rather than market the project: take-home assessments, interview submissions, pre-submission audits, and architecture reviews. Triggered by `/readme --variant assessment` and called from `/assessment` step 12.
...
For the marketing-grade variant covering open-source repos and portfolio projects, see the default flow documented in `SKILL.md` in this directory.
...
A scannable table that lets the reviewer see at a glance what was deferred
and why. Stripe, Cloudflare, and similar engineering-rigor employers use the
take-home as a starting point for the final loop conversation; this table
becomes the agenda for that discussion.

| What I would do | Why I did not do it now | Risk if shipped as is |
|-----------------|-------------------------|----------------------|
| [Concrete improvement] | [Time, scope, or dependency] | [What could go wrong in production] |
| [Concrete improvement] | [Time, scope, or dependency] | [What could go wrong in production] |
| [Concrete improvement] | [Time, scope, or dependency] | [What could go wrong in production] |

At least 3 rows. Concrete improvements, not aspirations. "Cache layer
with Redis for the hot endpoints" beats "performance improvements." The
Risk column makes clear what the reviewer should worry about.

## Assumptions

A short list of explicit assumptions made during the implementation. These
are the questions the reviewer would otherwise have to ask, surfaced
proactively. Include only the non-obvious ones.

- [Assumption about traffic shape, data volume, or scale]
- [Assumption about user behavior or input distribution]
- [Assumption about deployment environment]
- [Assumption about the assessment scope, e.g., "Treated the API as the
  trust boundary; auth left to the gateway since the prompt did not name
  authentication as in scope"]
