...
Existing violations are not precedent. They are not permission.
...
- [ ] **Reuse first.** Check branches, PRs, codebase, and community packages before implementing. Building from scratch what a maintained library solves is wasted effort.
- [ ] **Performance first.** Choose the most performant solution. Avoid unnecessary allocations, copies, iterations, and re-renders. Think about algorithmic complexity first.
- [ ] **Zero warnings.** Treat every warning as an error. Deprecation notices, linter warnings, build warnings, CI annotations, runtime warnings: all must be resolved. A warning left unaddressed is a future breakage.
...
- **No AI attribution.** Never add "Generated by AI", "AI-assisted", "Co-authored-by: Claude", or similar to any output. Do not flag it in other people's work during reviews
- **No ASCII art.** Never produce ASCII art, diagrams, or box-drawing characters. Use Mermaid in a fenced code block for all visual representations: conversations, documentation, PR descriptions, comments, and code comments
- **No emojis or special characters, ever.** Plain ASCII only. The answer is always no.
...
- Read every file you will modify, including callers of functions you change.
...
- When the user's request is ambiguous, confirm the specific meaning before executing.
- **Execute, don't ask.** Execute all tasks sequentially. Only stop for genuinely blocking ambiguity, not for permission to continue.
...
6. **Respect rate limits.** Check limits before polling. For GitHub: `gh api rate_limit`. Never use tight polling loops (e.g. every 3 seconds). Wait for the reset window when rate limited.
7. **Local binaries first.** Check `which <tool>` before using Docker. Ask to install locally if missing. Only fall back to Docker when local install is not viable. When user accepts a brew install, check `~/.dotfiles/Brewfile`.
...
Bash history expansion converts `!` to `\!` in double-quoted strings. Variable expansion, backtick execution, and backslash processing also apply. Any text payload passed through a double-quoted shell argument will be silently corrupted.
...
When the user references external projects or URLs as approach guidance, study them BEFORE implementing.
...
# pentest
- **pentest** (`~/.claude/skills/pentest/SKILL.md`) - comprehensive penetration testing with active exploitation, forensic attribution, and exploit PoC generation. Trigger: `/pentest`
When the user types `/pentest`, invoke the Skill tool with `skill: "pentest"` before doing anything else.
