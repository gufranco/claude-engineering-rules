...
- **No AI process leak.** Beyond the explicit attribution rule above, never publish process language in artifacts other humans read: commit messages, PR descriptions, release notes, code comments, chat messages. Forbidden phrases include phase-N, references to the planning doc, paths inside the project planning folder, canvas-region mappings, casual ADR-N references, and hyperbole like "state-of-the-art" or "100% faithful". Full rule: `~/.claude/rules/no-ai-process-leak.md`. Enforced by `~/.claude/hooks/ai-process-leak-blocker.py`. Bypass `AI_PROCESS_LEAK_DISABLE=1` (parent shell export only) when editing planning artifacts.
