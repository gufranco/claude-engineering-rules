...
- [`scripts/markdown_link_detector.py`](scripts/markdown_link_detector.py): shared detection module used by both the validator and the hook so they cannot drift. Detects backticked tokens whose path resolves to an existing repo file and which are not wrapped in a markdown link. Skip-list for [`tests/`](tests/), [`scripts/`](scripts/), [`.github/`](.github/), `tools/`, and `specs/`. The `specs/` tree is advisory only.
