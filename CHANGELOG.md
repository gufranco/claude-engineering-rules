...
- `/audit trust` subcommand at [`skills/audit/SKILL.md`](skills/audit/SKILL.md). Read-only safety scan for untrusted projects. Detects install-time hooks, credential-theft patterns, exfiltration endpoints, Discord, Telegram, pastebin, CI/CD attack patterns, `pull_request_target` plus PR-ref checkout, editor and tooling auto-run configs, dependency red flags, typosquats, age, lockfile drift, non-default registry, and binary anomalies. Produces a four-tier verdict: SAFE / SUSPICIOUS / HIGH-RISK / MALICIOUS. Auto-detects gitleaks, semgrep, trivy, and `npm audit signatures` for Sigstore provenance verification.
- [`skills/audit/trust-patterns.md`](skills/audit/trust-patterns.md): IOC catalog. Eight sections covering install hooks, code red flags, sensitive file access, network exfiltration, CI/CD, editor auto-run, dependency red flags, and binary anomalies. Each pattern carries a severity of CRITICAL, HIGH, MEDIUM, or LOW, plus target, rationale, and known false positives. Append-only. Refreshed quarterly. Seeds known-malicious package list with names from Shai-Hulud, Axios compromise, Mini Shai-Hulud, TrapDoor, durabletask, PyTorch Lightning, and Discord-webhook-C2 campaigns.
...
- [`hooks/_lib/markdown_link_detector.py`](hooks/_lib/markdown_link_detector.py): shared detection module used by both the validator and the hook so they cannot drift. Detects backticked tokens whose path resolves to an existing repo file and which are not wrapped in a markdown link. Skip-list for [`tests/`](tests/), [`.github/`](.github/), [`tools/`](tools/), and `specs/`. The `specs/` tree is advisory only.
- [`.github/scripts/validate-markdown-links.py`](.github/scripts/validate-markdown-links.py): CI validator. Scans every tracked `.md` file. Exit 1 on any bare reference to an existing repo file. Wired into the Lint job in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
- [`.github/scripts/fix-markdown-links.py`](.github/scripts/fix-markdown-links.py): one-shot auto-wrapper used to apply the rule across legacy markdown. Applied 522 fixes across 100 files in this rollout.
...
- Self-review-driven fixes: `bulk-resolve-blocker.py` rewritten to require an actual `gh api ... resolveReviewThread` or `glab api -X PUT ... resolved=true` call before counting, eliminating a false-positive that fired whenever a Bash command happened to mention `resolveReviewThread` alongside an unrelated loop. GitLab REST-style bulk resolves are now also detected. README hook count corrected to 43 in both the sidebar metric and the project-tree block. Docstrings on the three new hooks now document their GitHub-only or GitHub-plus-GitLab platform scope explicitly. `.last-cleanup` added to [`.gitignore`](.gitignore) to stop it from showing in `git status`
...
- 3 new hooks: notify-webhook, Stop, compact-context-saver for PreCompact and PostCompact, failure-logger. PostToolUseFailure
