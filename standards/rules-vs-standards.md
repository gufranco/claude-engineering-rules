...
1. **Domain-specific.** Applies only to a technology, framework, or workflow. Example: [`postgresql.md`](postgresql.md), [`redis.md`](redis.md), [`rust.md`](rust.md).
2. **Reference material.** A lookup the model consults rather than guidance it always carries. Example: [`llm-docs.md`](llm-docs.md), [`identifiers.md`](identifiers.md).
...
| Document each MCP server's purpose and trigger set in [`mcp-security.md`](mcp-security.md) | Otherwise nobody remembers what it does |
