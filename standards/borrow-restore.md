...
No CLI in the user's current toolchain requires this pattern. Every tool the user runs supports either a per-command form (see [`standards/multi-account-cli.md`](multi-account-cli.md)) or per-project config files that resolve on every invocation without global mutation.
...
For account and context detection, follow the order in [`standards/multi-account-cli.md`](multi-account-cli.md) (env var, project config, git remote, ask). The same order applies when borrow-and-restore is the only option.
