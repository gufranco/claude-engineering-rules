...
- **Run the formatter over the same scope CI runs it over.** CI formats the whole project. Passing an explicit list of the files just edited proves nothing about the files CI will also check, and it passes locally while CI fails. Two ways the gap opens: a file that another tool rewrote as a side effect, such as an ESLint suppressions manifest regenerated by `--prune-suppressions`, and a newly tracked file that was ignored when the earlier pass ran. Both are invisible to a file-list run and both fail CI. Run `prettier --write .` followed by `prettier --check .`, or the project equivalent, and read the check output before committing.
