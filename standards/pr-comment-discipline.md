...
Two prose layers already govern comment voice. [`../standards/code-review.md`](code-review.md) forbids labels, templates and checklist shape; [`anti-slop.md`](../rules/anti-slop.md) catalogues the rhetorical shapes and records a corpus calibration that returned no findings across 209 of this repository's own Markdown files. Where a person chooses whether to write at all, the voice holds.
...
[`../standards/pr-comment-channels.md`](pr-comment-channels.md) exists because an inline-only sweep missed a blocking report. Every channel is still fetched, still classified, and still acted on. Only the shape of the answer changed.
...
The failure-scenario gate in [`ai-review-convergence.md`](../rules/ai-review-convergence.md) still decides whether a finding is real: write the concrete inputs and the wrong output first, check them against the code, and fix only what survives. What changes is the closure. A surviving finding is closed by the commit that fixes it. A finding that does not survive is closed by resolving the thread.
...
| A code marker recording the debt | Fix it. Markers are banned by [`code-style.md`](../rules/code-style.md) |
...
- [`../standards/pr-comment-channels.md`](pr-comment-channels.md): what a pull-request comment is on each platform, and the fetch that stays mandatory.
- [`../standards/code-review.md`](code-review.md): comment voice, and the internal taxonomy that never reaches a reader.
- [`ai-review-convergence.md`](../rules/ai-review-convergence.md): the failure-scenario gate and the stop condition for a review loop.
- [`anti-slop.md`](../rules/anti-slop.md): the shapes a reply must not take.
- [`found-fix.md`](../rules/found-fix.md): a problem surfaced by any verification surface is in scope for the current change.
- [`no-ai-process-leak.md`](../rules/no-ai-process-leak.md): what a reply must never disclose about how it was produced.
