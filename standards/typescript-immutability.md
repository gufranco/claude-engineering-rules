...
Outside these scopes, the rule is absolute. There is no allow marker: the hook honors third-party tool directives (`eslint-disable`, `@ts-expect-error`, `@ts-nocheck`) because another tool parses them, and nothing else. A comment invented to silence our own hook would be banned by the comments policy in [`../code-style.md`](../rules/code-style.md), so the escape hatch lives out-of-band instead.
...
The boundary legs have their own standards. [`../../standards/idempotency.md`](idempotency.md) carries key sourcing, fingerprints, and replay. [`../../standards/concurrency.md`](concurrency.md) carries the race taxonomy and the correctness ladder, including the races immutability does not remove. [`../../standards/immutability.md`](immutability.md) extends this file's discipline to Python, Go, Rust, JVM, and C#, and to stored data.
