...
- Cross-aggregate consistency is eventual. Use domain events to synchronize. For multi-step cross-aggregate workflows, use the Saga pattern from [`standards/distributed-systems.md`](distributed-systems.md)
- Validation lives in domain objects and application services. The infrastructure layer does NOT validate business rules. See [`standards/hexagonal-architecture.md`](hexagonal-architecture.md) for the boundary definition.
