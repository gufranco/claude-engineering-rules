...
| Transcript retention | Derived notes are the default artifact. Full transcript retention is opt-in per source folder, with a 24-month default and a sweep that enforces it, per [`privacy-defaults.md`](../rules/privacy-defaults.md) |
...
- [`memory-supersede.md`](../rules/memory-supersede.md): the supersede chain this rule gives a mechanism to.
- [`verification.md`](../rules/verification.md): evidence over assertion. Retrieval quality is measured, never assumed.
- [`normative-keywords.md`](../rules/normative-keywords.md): the obligation vocabulary used above.
- [`writing-precision.md`](../rules/writing-precision.md): every note passes the same precision gate as any other authored text.
- [`privacy-defaults.md`](../rules/privacy-defaults.md): third-party personal data in transcripts, retention, and minimization.
- [`language.md`](../rules/language.md): notes are written in English regardless of the language of the conversation that produced them.
- [`markdown-links.md`](../rules/markdown-links.md): file references inside the vault are links, not bare paths.
