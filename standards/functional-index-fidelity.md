...
Loaded when a task writes a query whose predicate wraps a column in a function or expression, or when a task adds an expression index. Triggers in [`index.yml`](../rules/index.yml).
...
- [`../standards/database.md`](database.md): general query and schema concerns.
- [`../standards/postgresql.md`](postgresql.md): PostgreSQL specifics.
- [`code-style.md`](../rules/code-style.md): the single-source-of-truth principle the shared helper applies.
- [`verification.md`](../rules/verification.md): why the plan rather than the result is the evidence here.
