...
| A verification gate | A waived gate is an unverified claim; see [`found-fix.md`](../rules/found-fix.md) |
...
[`rule-provenance.md`](../rules/rule-provenance.md) closes the loop. Waivers accumulating against the same rule for the same reason are evidence the rule is mis-scoped, and that is a pruning or rewriting trigger, not a reason to keep granting waivers.

[`surgical-edits.md`](../rules/surgical-edits.md) is unaffected. A waiver widens what is permitted at one point, never what may be changed.

[`no-ai-process-leak.md`](../rules/no-ai-process-leak.md) governs the wording. A waiver in a pull request body is written as an engineering note, not as a compliance artifact.
