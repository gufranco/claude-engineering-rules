...
Cross-language baselines live in [`rules/code-style.md`](../rules/code-style.md). This file covers Python-specific tooling, typing, project layout, testing, and async patterns. Triggered when the task names Python or the project ships a `pyproject.toml`.
...
The same logic that drives [`typescript-immutability.md`](../rules/lang/typescript-immutability.md) applies: parameters and return values are immutable views; mutation lives behind a function boundary.
...
- Use real databases for integration tests, not mocks. See the integration-first philosophy in [`rules/testing.md`](../rules/testing.md)
...
The strict mock policy from [`rules/testing.md`](../rules/testing.md) applies: mock external third-party APIs, time, and randomness. Never mock the database, Redis, the project's own services, or modules under test.
...
- Re-raise after logging, unless the catch site is the final boundary. The error classification rule in [`rules/code-style.md`](../rules/code-style.md) applies
...
- [`rules/code-style.md`](../rules/code-style.md): cross-language fundamentals, error classification, validation at boundaries
- [`rules/testing.md`](../rules/testing.md): integration-first philosophy, strict mock policy
- [`rules/architecture-defaults.md`](../rules/architecture-defaults.md): DDD, hexagonal, idempotency baseline
- [`standards/database.md`](database.md): generic database concerns
- [`standards/postgresql.md`](postgresql.md): PostgreSQL specifics, including PG 18 native `uuidv7()`
