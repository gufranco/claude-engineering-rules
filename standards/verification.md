...
- Dismissing a flagged issue as "pre-existing" or "not introduced by this change". A problem surfaced by a verification surface is in scope regardless of when it was introduced. See [`found-fix.md`](../rules/found-fix.md)
- Trusting an existence oracle that was never calibrated. Many web indexes answer HTTP 200 with a "not found" page, and many commands exit 0 on a miss, so a naive check reports every candidate as present. Before running the check across a list, probe it both ways: a name that cannot exist, confirming it reports absent, and one that certainly exists, confirming it reports present. Calibrating in one direction only leaves the mirror failure open, where a query naming a field the source does not return, or a pattern that cannot match, yields an empty result indistinguishable from a genuinely clean answer. A reported count needs the same check: confirm that what is counted is what is reported, since lines and objects differ whenever one object spans several lines
...
- Assuming a blocked tool call partially applied. A PreToolUse block runs nothing, so every change that call carried is still unmade, including the parts that precede the offending one. Re-read the file instead of re-running only the piece that tripped the hook, and when the blocked command staged or set up state for its own later part, repeat the whole command rather than the tail, which would otherwise run against state left by an earlier call
...
- Treating a check that failed to run as a check that was skipped. A classifier that exits non-zero, a gate whose output will not parse, a state file that is missing, and a tool that errored are all failed checks, never absent ones. The asymmetry decides it: failing closed costs a false stop, which is visible and cheap to clear, while failing open costs an unverified change that looks verified. Full rule, including the exit-code and stream mechanics that make fail-closed real rather than nominal, in [`agent-operating-limits.md`](../rules/agent-operating-limits.md)
- Answering a time-dependent question from the date stamped into session context. That value is written once at session start and never updated, so it is wrong in any session that outlives a day boundary and in any resumed session. Run `date` and read the output before reasoning about today, a deadline, an age, or an elapsed interval
- Publishing a specific that a subagent, a search snippet, or a summary reported, without opening the primary source at that coordinate. Relays garble the coordinate, not only the content, so the quoted text reads correctly while the line number, timestamp, or identifier points somewhere else. See [`relay-not-source.md`](../rules/relay-not-source.md)
...
**Frontend and mobile changes**: render it. A passing unit test in a DOM emulation is not render evidence, because that layer implements no paint, no real cascade, and no full accessibility tree. Drive the project's browser or simulator suite, or `agent-browser open` plus `eval` for computed style and `snapshot -i` for the tree. Naming which checks were skipped is acceptable; implying they happened is not. See [`frontend-render-gate.md`](../rules/frontend-render-gate.md).
