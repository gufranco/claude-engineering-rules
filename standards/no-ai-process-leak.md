...
- `gh pr create`, `gh pr edit`, `gh issue create/edit/comment`, `gh release create/edit`, and a reply posted into a review thread. The commands that publish a pull-request comment outside a thread are banned outright by [`pr-comment-discipline.md`](../rules/pr-comment-discipline.md), so this rule never reaches them
