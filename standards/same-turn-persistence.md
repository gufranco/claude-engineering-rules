...
|---|---|
| How you should work, generally | A `feedback` memory, or an existing rule in [`rules/`](../rules) when the correction sharpens one |
| A fact about the user | A `user` memory |
| A fact about ongoing work | A `project` memory, or the vault per [`knowledge-notes.md`](../rules/knowledge-notes.md) |
...
A correction that fits an existing file updates that file. It does not create a second file saying almost the same thing. See [`memory-supersede.md`](../rules/memory-supersede.md).
...
[`rule-provenance.md`](../rules/rule-provenance.md) governs what happens when a persisted lesson recurs often enough to become a binding rule rather than a memory entry.

[`memory-supersede.md`](../rules/memory-supersede.md) governs updating a memory that already exists instead of overwriting or duplicating it.

[`writing-precision.md`](../rules/writing-precision.md) governs how the line is written once the admission bar is cleared.
