...
Never rely on "it will expire eventually" for data that users expect to see updated immediately after a write. The acceptable staleness window must align with the consistency model chosen for the system. Align with the consistency model hierarchy in [`standards/distributed-systems.md`](distributed-systems.md).
...
- If pre-loading is not feasible, rate-limit cache-miss paths to protect the origin during warmup. See [`standards/resilience.md`](resilience.md) Back Pressure for rate limiting patterns
