#!/usr/bin/env python3
"""
typeorm-raw-sql-blocker.py

PreToolUse hook that blocks TypeORM raw query methods in application code.
Rule source: ~/.claude/rules/code-style.md "No raw SQL"
            + ~/.claude/rules/lang/typeorm-migrations.md.

Blocks .query() called on the typical TypeORM connection-like identifiers:
  - dataSource.query(...)
  - manager.query(...)
  - entityManager.query(...)
  - queryRunner.query(...) outside migrations
  - connection.query(...)
  - repository.query(...)
...
PATTERN = re.compile(
    r"\b(?:dataSource|appDataSource|connection|conn|manager|entityManager|"
...
def main() -> int:
    if not should_run("typeorm-raw-sql-blocker"):
        _sys.exit(0)
    if os.environ.get("TYPEORM_RAW_SQL_DISABLE") == "1":
        _audit(
    ...
    print(
        "Blocked: TypeORM raw .query() is banned in application code. "
        'Rule: ~/.claude/rules/code-style.md "No raw SQL".\n'
        + "\n".join(findings)
        + "\n\nFix: express the query using TypeORM's query builder "
        "(repository.createQueryBuilder(), find(), findOne(), update(), delete()). "
        "Raw .query() is only allowed in migration files and operational scripts.\n"
        "Bypass (when there is genuinely no query builder equivalent): "
        "set TYPEORM_RAW_SQL_DISABLE=1.",
        file=sys.stderr,
    )
    _audit(
        hook="typeorm-raw-sql-blocker",
