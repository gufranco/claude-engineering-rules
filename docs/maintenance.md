...
- [ ] Verify all hooks in [`settings.json`](../settings.json) reference files that still exist in [`hooks/`](../hooks)
...
- [ ] Read every file in [`rules/`](../rules), remove rules that are now obvious defaults in Claude Code
- [ ] Read every file in [`standards/`](../standards), check that examples match current library versions
- [ ] Review [`rules/index.yml`](../rules/index.yml) trigger keywords, add or remove entries for changed rules and standards
...
- [ ] Check that all agents in [`agents/`](../agents) appear in the agent table in README.md
- [ ] Verify [`agents/TEMPLATE.md`](../agents/TEMPLATE.md) reflects any new frontmatter fields added since last quarter
...
- [ ] Audit [`checklists/checklist.md`](../checklists/checklist.md), add items for new OWASP entries or updated CWE classifications
- [ ] Review each skill in [`skills/`](../skills), check subcommand examples against current Claude Code behavior
- [ ] Run each skill with `--help` equivalent and verify it matches the documentation in README.md
- [ ] Check [`skills/security-patterns.md`](../skills/security-patterns.md) for new OWASP entries, CWE updates, or emerging vulnerability classes

### Security review

- [ ] Run [`hooks/secret-scanner.py`](../hooks/secret-scanner.py) against the `~/.claude/` directory itself, ensure no secrets landed in config files
- [ ] Verify all `mcpServers` entries in [`settings.json`](../settings.json) use `${ENV_VAR}` for credentials, never hardcoded values
- [ ] Check that [`.gitignore`](../.gitignore) excludes all sensitive file patterns
...
| New MCP server released | Evaluate against existing servers, add to [`settings.json`](../settings.json) if it adds distinct capability |
