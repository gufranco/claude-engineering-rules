...
| [`pr-comment-discipline`](rules/pr-comment-discipline.md) | The only comment published on a pull request is a reply inside a thread a person opened. No summary body, no conversation comment, no commit-comment reply, no reply of any length to a bot. Bot threads are read, fixed when real, then resolved in silence. Replies carry a four-sentence ceiling, and anything fixable in the open change is fixed there rather than filed |
...
| [`comment-blocker.py`](hooks/comment-blocker.py) | PreToolUse Write/Edit/MultiEdit | Blocks any prose comment added to source code, test files included. An edit is resolved against the file on disk and the resulting document is scanned, so a fragment that opens or closes a docstring is read in context and only newly added comments are reported. Only tool directives (`eslint-disable`, `@ts-expect-error`, `go:build`, `noqa`, `type: ignore`, `shellcheck disable`) are exempt. No suppression marker; operator bypass `COMMENT_BLOCKER_DISABLE=1` |
...
| [`pr-comment-discipline.py`](hooks/pr-comment-discipline.py) | PreToolUse Bash | Blocks every command that publishes a pull-request comment outside a thread: the `gh pr` comment subcommand, `gh pr review` with a body, a reviews POST carrying a non-empty summary, a `pulls/<n>/comments` POST with no `in_reply_to`, an `issues/<n>/comments` POST, a commit-comment POST, a GitLab MR note or new discussion, and a parentless Bitbucket comment. Reads `--input` and `--body-file` payloads. Leaves thread replies, empty-body review submissions, and description edits alone. Bypass `PR_COMMENT_DISCIPLINE_DISABLE=1` |
...
| [`tdd-gate.py`](hooks/tdd-gate.py) | PreToolUse Write/Edit/MultiEdit | Blocks creation of a new production source file when no companion test file can be located. Exempts config files, `__init__.py`, `conftest.py`, `setup.py`, type-only modules, re-export-only `index` barrels, and throwaway scripts under a `scratchpad/` directory or the shared `/tmp` root. The per-user `TMPDIR` stays gated, since pytest's `tmp_path` lives there. Bypass `TDD_GATE_DISABLE=1` |
