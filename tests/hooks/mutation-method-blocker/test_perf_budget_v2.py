...
def test_hook_meets_v2_perf_budget(hook_path):
    """p95 < 1000ms, p99 < 1500ms, mean < 600ms (subprocess + parallel-inclusive)."""
    # Arrange
    ...
    assert p95 < 1000.0, f"p95 {p95:.1f}ms exceeds 1000ms"
    assert p99 < 1500.0, f"p99 {p99:.1f}ms exceeds 1500ms"
    assert mean < 600.0, f"mean {mean:.1f}ms exceeds 600ms"
