...
def run(payload: dict[str, str], env: dict[str, str]) -> subprocess.CompletedProcess[str]:
...
def test_the_slug_matches_how_claude_code_names_a_project(tmp_path: Path) -> None:
    ...
    memory = module.memory_dir_for(Path("/Users/someone/Dropbox/Workspace/@ Pessoal/battle"))
...
def test_no_vault_configured_is_silent(tmp_path: Path) -> None:
    result = run({"cwd": str(tmp_path)}, {"PATH": "/usr/bin:/bin", "HOME": str(tmp_path)})
...
def test_a_broken_compile_never_fails_the_session(tmp_path: Path) -> None:
    ...
    (vault / ".ci" / "compile.py").write_text("import sys; sys.exit(3)", encoding="utf-8")
